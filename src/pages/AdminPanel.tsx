...
const AdminPanel: React.FC = () => {
  ...
  // Pagination for products (optimized - no more mass loading)
  ...
  // Load products with pagination (optimized)
  const loadProducts = async (page: number = 1) => {
    try {
      ...
      setProductsTotalPages(Math.ceil((response.total || 0) / productsPerPage));
    } catch (error) {
  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      // Check if all basic APIs failed
      const allBasicApiFailed = results.every(
        (result) => result.status === "rejected"
      );
      if (allBasicApiFailed && products.length === 0) {
