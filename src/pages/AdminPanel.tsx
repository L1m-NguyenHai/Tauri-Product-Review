...
import { BlockUserModal, UnblockUserModal } from "../components/BlockUserModal";
import {
  blockedUsersService,
  type BlockedUser,
...
const AdminPanel: React.FC = () => {
  ...
  const [prefilledRequestData, setPrefilledRequestData] =
  ...
  const [selectedUserForUnblock, setSelectedUserForUnblock] = useState<{
    userId: string;
    userName: string;
  } | null>(null);
  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      const results = await Promise.allSettled([
        ...
        adminAPI.getAllReviewsForAdmin({
          limit: 10,
          sort_by: "created_at",
          sort_order: "desc",
        }),
      ...
      const allBasicApiFailed = results.every(
  ...
  // Block user function - open modal instead of prompt
  const handleBlockUser = async (user: User) => {
    setSelectedUserForBlock(user);
    setShowBlockUserModal(true);
  };

  // Block user confirmation from modal
  const handleBlockUserConfirm = async (user: User, reason?: string) => {
    setBlockActionLoading(true);
    try {
      await blockedUsersService.blockUser(
        user.id,
        user.email,
        user.name,
        reason
      );
      await loadBlockedUsers(); // Refresh blocked users list
      showNotification("success", `User ${user.name} has been blocked`);
  ...
  // Unblock user function - open modal instead of confirm
  const handleUnblockUser = async (userId: string, userName: string) => {
    setSelectedUserForUnblock({ userId, userName });
    setShowUnblockUserModal(true);
  };

  // Unblock user confirmation from modal
  const handleUnblockUserConfirm = async (userId: string, userName: string) => {
    setBlockActionLoading(true);
    try {
      await blockedUsersService.unblockUser(userId);
      await loadBlockedUsers(); // Refresh blocked users list
      showNotification("success", `User ${userName} has been unblocked`);
    ...
    ? [
        {
          ...
          change: `+${dashboardStats.recent_registrations || 0}`,
        },
        {
          icon: MessageSquare,
          label: "Total Reviews",
          value: dashboardStats.total_reviews?.toLocaleString() || "0",
          change: `${dashboardStats.pending_reviews || 0} pending`,
        },
        {
          icon: Package,
          label: "Products",
  ...
  return (
    <div className="space-y-8">
                              ...
                              <select
                                ...
                                onChange={(e) =>
                                  handleUpdateUserRole(
                                    user.id,
                                    e.target.value as "user" | "reviewer"
                                  )
                                }
          ...
          {activeTab === "blocked-users" && (
            <div>
              <div className="flex justify-between items-center mb-6">
                ...
                >
                  Blocked Users ({blockedUsers.length} blocked)
                </h3>
                <div className="flex gap-2">
                  <button
                    onClick={() =>
                    ...
                    disabled={loading || blockedUsers.length === 0}
                    className="flex items-center space-x-2 px-3 py-2 text-sm bg-red-500 text-white rounded-lg hover:bg-red-600 disabled:opacity-50 transition-colors"
                  >
          ...
          {activeTab === "reviews" && (
            <div>
              ...
              <div className="mb-4 text-sm text-gray-500">
                Reviews loaded: {reviews.length} | Loading:{" "}
                {loading ? "Yes" : "No"}
              </div>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {request.status === "pending" && (
                            <>
                              ...
                              <button
                                onClick={() =>
                                  handleApproveAndCreateProduct(request)
                                }
      ...
      <BlockUserModal
        ...
        onBlock={handleBlockUserConfirm}
      ...
      <UnblockUserModal
        ...
        userId={selectedUserForUnblock?.userId || ""}
        userName={selectedUserForUnblock?.userName || ""}
        onUnblock={handleUnblockUserConfirm}
        loading={blockActionLoading}
      />
    </div>
  );
