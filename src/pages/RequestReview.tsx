...
import { Package, Link, DollarSign, Tag, FileText, Send, ChevronLeft, ChevronRight, Check } from 'lucide-react';
...
const RequestReview: React.FC = () => {
  ...
  const validateStep = (step: number): boolean => {
    switch (step) {
      case 1:
        return !!(formData.productName && formData.manufacturer && formData.category);
      case 2:
  ...
  const nextStep = () => {
    if (validateStep(currentStep)) {
      ...
      showNotification('warning', 'Please fill in all required fields before proceeding');
    }
  };
                ...
                {formData.reasoning && (
                  <div>
                    ...
                    <p className={`mt-1 ${isDark ? 'text-white' : 'text-gray-900'}`}>
                      {formData.reasoning.substring(0, 200)}{formData.reasoning.length > 200 ? '...' : ''}
                    </p>
                  </div>
                )}
              </div>
